...
def load_and_prepare_data(filename):
    ...
    print(f"✓ Loaded {len(df)} schedules from {filename}")
...
def analyze_by_base(df):
    ...
    print(f"\n✓ Saved base analysis to: {output_file}\n")
...
def calculate_correlations(df):
    ...
    Strong correlations (near ±1) indicate variables that move together
    """
    print("=" * 70)
...
def build_model_1(df):
    ...
    print(f"\n  → Each additional credit hour adds ~${model.coef_[0]:.2f} to cost\n")
...
def build_model_2(df):
    ...
    Cost = β₀ + β₁×credits + β₂×duties + β₃×vacations
...
def build_model_3(df):
    ...
    Cost = β₀ + β₁×credits + β₂×duties + β₃×vacations + β₄×(BASE2) + β₅×(BASE3)
...
def build_model_4(df):
    ...
    Cost = β₀ + β₁×credits + β₂×duties + β₃×vacations + β₄×(credits × duties)
    ...
    """
    print("=" * 70)
    print("MODEL 4: Cost ~ Credits + Duties + Vacations + (Credits × Duties)")
    print("=" * 70)
...
def compare_models(models_info):
    ...
    print("  R²: Proportion of variance explained (higher is better, 0-1 scale)")
    print("  Adj R²: R² adjusted for number of parameters (penalizes complexity)")
    print("  RMSE: Average prediction error in dollars (lower is better)")
    print("  Num_Parameters: Number of coefficients in the model")
    print("\n  → Look for high R² with low RMSE")
    print("  → Adjusted R² helps compare models with different complexity\n")
...
def identify_outliers(df, residuals, threshold=1000.0):
    ...
    if len(outliers) > 0:
        ...
        print(f"\n✓ Saved outlier details to: {output_file}")
    else:
...
def analyze_extra_credits(df):
    ...
    print("\n💡 Insight: Extra credits likely include:")
