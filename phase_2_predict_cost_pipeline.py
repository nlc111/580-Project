...
It ingests raw flight-leg data from daily CSV files, schedule and pairing definitions from text-based solver 
outputs, and externally credited cost summaries. Each pairing is transformed into a structured feature 
representation capturing duty structure, timing characteristics, and inherited schedule context. 
A regularized polynomial regression model (Ridge) is then trained using leave-one-solution-out cross-validation 
to predict pairing costs while limiting overfitting across solution instances.

The final output consists of predicted costs attached to each pairing, enabling downstream use in optimization, 
ranking, or schedule refinement workflows.
Input data:
...
def parse_schedules_from_file(schedules_file):
    """
    Reads a text-based solution file and extracts schedule-level metadata, including base, assigned 
    employee, ordered activity list, and basic counts such as number of activities and vacations. 
    The output dictionary serves as the structural link between raw schedules and downstream pairing analysis.
    """
    schedule_dict = {}
...
def load_credited_hours(file_path):
    """
    Parses an externally generated summary file containing credited hours, total schedule cost, and vacation 
    counts. The resulting mapping is used to attach observed outcomes to schedules for validation and regression 
    targets.
    """
    schedules = {}
...
def attach_schedule_info_to_pairings(pairings, credited_dict):
    """
    Enriches pairing records with schedule-level attributes (credited hours, total cost, vacation count) 
    when a pairing can be linked to a known schedule. Pairings that cannot be matched are assigned zero-valued 
    defaults to allow continued processing.
    """
...
def pairing_to_features(pairing, leg_dict, schedule_dict):
    """
    Converts a single crew pairing into a fixed-length feature vector suitable for regression. Features capture 
    structural properties (number of duties, deadheads), temporal properties (durations, gaps, start/end times), 
    and inherited schedule context when available.
    """
...
def build_feature_dataframe(pairings, leg_dict, schedule_dict):
    """
    Applies feature extraction across all pairings to construct the regression design matrix. 
    Also collects target values and grouping labels used for cross-validation, ensuring consistent alignment 
    between features, costs, and solution instances.
    """
    feature_rows = []
    targets = []
    groups = []
...
def train_and_cv(X, y, groups, degree=2, alpha=1.0):
    """
    Trains a Ridge regression model with polynomial feature expansion and categorical encoding. 
    Uses leave-one-solution-out cross-validation to reduce information leakage across solution instances, 
    then fits a final model on the full dataset.
    """
...
def predict_costs(model, poly_transformer, ohe, numeric_cols, categorical_cols,
    ...
    """
    Applies the trained regression model to generate cost predictions for each pairing. 
    Predicted values are written back into the pairing records for downstream inspection or optimization use.
    """
